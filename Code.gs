...
// 8. Copia la URL que te entrega y pégala en el HTML (variable APPS_SCRIPT_URL)

const SHEET_NAME = 'Respuestas'; // Nombre de la hoja donde se guardarán los datos
...
// Permite testear desde el navegador (GET)
function doGet() {
  return ContentService
...
function buildRow(d) {
  ...
  const autoMap = {
    ...
    b: 'Algunas, de forma parcial',
    c: 'La mayoría automatizadas'
  };
  const seguMap = {
    a: 'Hay seguimiento formal',
    b: 'Seguimiento irregular',
    c: 'Depende de cada persona',
    d: 'Generalmente no se registran'
  };
  const retraMap = {
    a: 'Casi nunca',
    b: 'Una o dos veces al mes',
    c: 'Varias veces por semana',
    d: 'Es parte del día a día'
  };
  const iaMap = {
    a: 'No usamos ninguna',
    b: 'Experimentamos sin sistematicidad',
    c: 'Sí, uso regular'
  };
