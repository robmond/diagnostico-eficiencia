...
// 5. La URL no cambia, no necesitas tocar el HTML
...
function doPost(e) {
  try {
    ...
    // Acepta tanto JSON como envío vía formulario (iframe)
    if (e.postData.type === 'application/json') {
      data = JSON.parse(e.postData.contents);
    } else {
      data = JSON.parse(decodeURIComponent(e.parameters.payload));
    }
